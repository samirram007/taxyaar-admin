...
import { useTicketStatus } from "../contexts/ticket-status-context"

interface DataTableRowActionsProps {
    row: Row<TicketStatus>
}

const RowActions = (props: DataTableRowActionsProps) => {
    const navigate = useNavigate()
    const { setOpen, currentRow, setCurrentRow } = useTicketStatus()
    const { row } = props
    return (
        <DataTableRowActions<TicketStatus>
            row={row}
            onEdit={(data) => {
                setCurrentRow(data)
